...
class Materialised_Measure(Base): 
    ...
    materialised_measure_id: so.Mapped[int] = so.mapped_column(primary_key=True)
    ...
    measure_people: so.Mapped[List['Measure_Person_Map']] = so.relationship(back_populates='materialised_measure')
...
class Measure_Person_Map(Base):
    ...
    materialised_measure_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey('materialised_measure.materialised_measure_id'))
...
class Materialised_Cohort(Base):
    ...
    materialised_cohort_id: so.Mapped[int] = so.mapped_column(primary_key=True) 
    id = so.synonym('materialised_cohort_id')
    ...
    materialised_cohort_defs: so.Mapped[List['Materialised_Cohort_Def']] = so.relationship(back_populates='materialised_cohort')

class Materialised_Cohort_Def(Base):
    __tablename__ = 'materialised_cohort_def'
    materialised_cohort_def_id: so.Mapped[int] = so.mapped_column(primary_key=True) 
    id = so.synonym('materialised_cohort_def_id')
    materialised_cohort_id: so.Mapped[int] =  so.mapped_column(sa.ForeignKey('materialised_cohort.materialised_cohort_id'))
    ...
    cohort_people: so.Mapped[List['Cohort_Person_Map']] = so.relationship(back_populates='materialised_cohort_def')

class Cohort_Person_Map(Base):
    __tablename__ = "cohort_person_map"
    
    materialised_cohort_def_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey('materialised_cohort_def.materialised_cohort_def_id'), primary_key=True)
    id = so.synonym('materialised_cohort_id')

    person_id: so.Mapped[int] =  so.mapped_column(sa.ForeignKey('person.person_id'), primary_key=True)
    cohort_date: so.Mapped[date] = so.mapped_column(sa.DateTime)

    materialised_cohort_def: so.Mapped['Materialised_Cohort_Def'] = so.relationship(foreign_keys=[materialised_cohort_def_id], back_populates='cohort_people')
    person: so.Mapped['Person'] = so.relationship("Person", foreign_keys=[person_id])
